...
for (i, (xg, yg)) ∈ enumerate(ranges)
    @time basins, attractors = basins_2D(xg, yg, integrator(he); mc_bas = 3)
    ...
    axs[i].plot(columns(tr)...; marker = "o", ms = 4, color = "C0", ls = "None")
...
for i in 1:2
    axs[i].plot(columns(tr)...; marker = "o", ms = 2, ls = "None", color = "C2")
end

axs[3].plot(0:N, tr[:, 1], marker = "o", ls = "None", ms = 5, color = "C2")
axs[3].set_ylim(-4, 2)
